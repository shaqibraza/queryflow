...
export class MysqlPromptBuilder implements PromptBuilder<PromptInput> {
...
Your job is to generate ONLY valid MySQL SQL.
          ...
          tableColumns.length === 0
            ...
            : tableColumns
                .map((column) => {
                  ...
                  const pk = pkColumns.includes(column.name) ? " PRIMARY KEY" : "";
                  ...
                  return `  • ${column.name} (${column.type})${nullable}${pk}${defaultValue}`;
                })
                .join("\n");
